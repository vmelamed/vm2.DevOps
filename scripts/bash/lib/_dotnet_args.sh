...
declare -xri __VM2_LIB_DOTNET_ARGS_SH_LOADED=1
...
#---------------------------------------------------------------------------------------------
# @description The default value for the `--configuration` option of the `dotnet build`
#   command. It is set to "Release" in CI/CD environments (`[[ $ci == true ]]`), otherwise -
#   "Debug".
#---------------------------------------------------------------------------------------------
declare -x default_configuration
$ci &&
...
#---------------------------------------------------------------------------------------------
# @description The default value for the `--define` option of the `dotnet build` command. It
#   is empty by default; consumers set it explicitly (e.g. to "SHORT_RUN") when they need to
#   enable preprocessor symbols.
#---------------------------------------------------------------------------------------------
declare -xr default_preprocessor_symbols=''
...
#   `dotnet build` command. It is set to an empty string - runtime agnostic.
#---------------------------------------------------------------------------------------------
declare -xr default_rid=''
...
#---------------------------------------------------------------------------------------------
# @description The default value for the `--artifacts-path` option of the `dotnet build`
#   command. Specifies the path to the directory where build artifacts are stored.
#---------------------------------------------------------------------------------------------
declare -xr default_artifacts_path="artifacts"
...
#=============================================================================================
# Define valid sets of values used to validate the command-line arguments whose names and
# roles are shared across vm2.DevOps scripts.
#=============================================================================================

#---------------------------------------------------------------------------------------------
# @description Valid values for the `--configuration` option of the `dotnet build` command in
#   the `vm2.*` projects.
#---------------------------------------------------------------------------------------------
declare -xra valid_configurations=(
    Debug
    Release
)

#---------------------------------------------------------------------------------------------
# @description Valid values for the `--framework` option of the `dotnet build` command in the
#   `vm2.*` projects.
#---------------------------------------------------------------------------------------------
declare -xra valid_tfms=(
    net9.0
    net10.0
)

#---------------------------------------------------------------------------------------------
# @description Valid values for the `--runtime` option of the `dotnet build` command in the
#   `vm2.*` projects.
#---------------------------------------------------------------------------------------------
declare -xra valid_rids=(
    ''              # runtime-agnostic output
    win-x64
    linux-x64
    osx-x64
    # also:
    # win-arm64, win-x86
    # linux-arm64, linux-musl-x64, linux-arm
    # osx-arm64
    # unix-x64, unix-arm64
    # ios-arm64
    # android-arm64
...
#---------------------------------------------------------------------------------------------
# @description Valid values for the `--verbosity` option of the `dotnet build` command in the
#   `vm2.*` projects. For the proper functioning of the `extract_dotnet_build_info()`
#   function, the verbosity level MUST be set to "detailed" or "diagnostic". Therefore the
#   `dotnet_build()` function enforces the verbosity level to be "detailed" and overrides any
#   other value specified in the build arguments.
#---------------------------------------------------------------------------------------------
...
#---------------------------------------------------------------------------------------------
# @description The default value for the `--verbosity` option of the `dotnet build` command.
#   It is set to "detailed" to ensure that the `extract_dotnet_build_info()` function can
#   properly extract build information from the output of the `dotnet build` command.
#---------------------------------------------------------------------------------------------
declare -xr default_verbosity="detailed"
...
declare -x preprocessor_symbols=${PREPROCESSOR_SYMBOLS:-$default_preprocessor_symbols}
declare -x configuration=${CONFIGURATION:-$default_configuration}
declare -x framework=${FRAMEWORK:-$default_tfm}
declare -x runtime=${RUNTIME:-$default_rid}
declare -x artifacts=${ARTIFACTS_PATH:-$default_artifacts_path}
declare -x minver_tag_prefix=${MINVERTAGPREFIX:-$default_minver_tag_prefix}
declare -x minver_prerelease_id=${MINVERDEFAULTPRERELEASEIDENTIFIERS:-} # maybe empty for release, so do not put here $default_minver_prerelease_id. The value must come from CLI.
