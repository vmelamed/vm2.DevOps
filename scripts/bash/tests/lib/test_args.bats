...
@test "get_common_arg: recognizes --verbose/-v and sets verbose mode" {
    run bash -c "source '$lib_dir/core.sh' --no-trap > /dev/null; get_common_arg --verbose; is_verbose"
    assert_success
    run bash -c "source '$lib_dir/core.sh' --no-trap > /dev/null; get_common_arg -v; is_verbose"
    assert_success
}

@test "get_common_arg: recognizes -q/--quiet and sets quiet mode" {
    run bash -c "source '$lib_dir/core.sh' --no-trap > /dev/null; get_common_arg --quiet; is_quiet"
    assert_success
}

@test "get_common_arg: recognizes -y/--dry-run and sets dry-run mode" {
    run bash -c "source '$lib_dir/core.sh' --no-trap > /dev/null; get_common_arg --dry-run; is_dry_run"
    assert_success
}

@test "get_common_arg: recognizes -gr/-md and sets table format" {
    run bash -c "source '$lib_dir/core.sh' --no-trap > /dev/null; get_common_arg -md; get_table_format"
    assert_success
    assert_output "markdown"
    run bash -c "source '$lib_dir/core.sh' --no-trap > /dev/null; get_common_arg -gr; get_table_format"
    assert_success
...
@test "get_common_arg: records --help/-h/-? into \$usage_requested" {
    run bash -c "source '$lib_dir/core.sh' --no-trap > /dev/null; get_common_arg --help; echo \"\$usage_requested\""
    assert_output "long"
    run bash -c "source '$lib_dir/core.sh' --no-trap > /dev/null; get_common_arg -h; echo \"\$usage_requested\""
    assert_output "short"
    run bash -c "source '$lib_dir/core.sh' --no-trap > /dev/null; get_common_arg -?; echo \"\$usage_requested\""
    assert_output "short"
}
...
@test "usage_if_requested: no-op when no usage was requested" {
    run bash -c "source '$lib_dir/core.sh' --no-trap > /dev/null; source '$lib_dir/_args.sh'; usage_if_requested; echo done"
    assert_success
    assert_output "done"
}

@test "usage_if_requested: exits (via usage) when --help was recorded" {
    run bash -c "source '$lib_dir/core.sh' --no-trap > /dev/null; source '$lib_dir/_args.sh'; get_common_arg --help; usage_if_requested; echo not-reached"
    assert_success
...
@test "usage: exits 0 with no error messages and no explicit exit code" {
    run bash -c "source '$lib_dir/core.sh' --no-trap > /dev/null; source '$lib_dir/_args.sh'; usage"
    assert_success
}

@test "usage: exits 1 (failure) when error messages are given but no explicit exit code" {
    run bash -c "source '$lib_dir/core.sh' --no-trap > /dev/null; source '$lib_dir/_args.sh'; usage true 'something went wrong'"
    assert_failure 1
...
@test "usage: preserves an explicitly-provided exit code, even alongside error messages" {
    run bash -c "source '$lib_dir/core.sh' --no-trap > /dev/null; source '$lib_dir/_args.sh'; usage true 42 'custom error'"
    assert_failure 42
...
@test "usage_text: prints the override-me placeholder, naming the script" {
    run bash -c "script_name='my-script.sh'; source '$lib_dir/core.sh' --no-trap > /dev/null; source '$lib_dir/_args.sh'; usage_text false"
    assert_success
