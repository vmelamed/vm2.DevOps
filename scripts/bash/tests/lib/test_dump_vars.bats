...
@test "_write_title: prints the header text" {
    run bash -c "source '$lib_dir/core.sh' --no-trap > /dev/null 2>&1; _current_table=graphical; _write_title 'My Header'"
    assert_success
...
@test "_write_line: prints a scalar variable's name and value" {
    run bash -c "source '$lib_dir/core.sh' --no-trap > /dev/null 2>&1; _current_table=graphical; myvar=hello; _write_line myvar false"
    assert_success
...
@test "_write_line: masks the value with \$secret_str when the secret flag is true" {
    run bash -c "source '$lib_dir/core.sh' --no-trap > /dev/null 2>&1; _current_table=graphical; myvar=topsecret; _write_line myvar true"
    assert_success
    assert_output --partial "myvar"
...
@test "_write_line: prints entry count and contents for an associative array" {
    run bash -c "source '$lib_dir/core.sh' --no-trap > /dev/null 2>&1; _current_table=graphical; declare -A myarr=([a]=1 [b]=2); _write_line myarr false"
    assert_success
...
@test "_write_line: prints item count and contents for an indexed array" {
    run bash -c "source '$lib_dir/core.sh' --no-trap > /dev/null 2>&1; _current_table=graphical; declare -a myarr=(x y z); _write_line myarr false"
    assert_success
...
@test "_write_line: prints '()' suffix for a defined function" {
    run bash -c "source '$lib_dir/core.sh' --no-trap > /dev/null 2>&1; _current_table=graphical; _write_line is_boolean false"
    assert_success
    assert_output --partial "is_boolean()"
}

@test "_write_line: uses a custom display name and treats \$1 as a literal value when unbound" {
    run bash -c "source '$lib_dir/core.sh' --no-trap > /dev/null 2>&1; _current_table=graphical; _write_line 'literal-value' false 'Custom Label'"
    assert_success
...
@test "_write_line: shows the unbound placeholder for an undefined name with no custom label" {
    run bash -c "source '$lib_dir/core.sh' --no-trap > /dev/null 2>&1; _current_table=graphical; _write_line definitely_not_defined_xyz false"
    assert_success
...
@test "dump_vars: --force dumps even when verbose is off" {
    run bash -c "source '$lib_dir/core.sh' --no-trap > /dev/null 2>&1; myvar=hello; dump_vars --force --quiet myvar"
    assert_success
    assert_output --partial "myvar"
    assert_output --partial "hello"
}
...
@test "dump_vars: --header prints the header text" {
    run bash -c "source '$lib_dir/core.sh' --no-trap > /dev/null 2>&1; myvar=hello; dump_vars --force --quiet --header 'My Section' myvar"
    assert_success
    assert_output --partial "My Section"
}

@test "dump_vars: --secret masks only the immediately following variable" {
    run bash -c "source '$lib_dir/core.sh' --no-trap > /dev/null 2>&1; secretvar=topsecret; plainvar=visible; dump_vars --force --quiet --secret secretvar plainvar"
    assert_success
    refute_output --partial "topsecret"
...
@test "dump_vars: --name assigns a custom display label to the next entry only" {
    run bash -c "source '$lib_dir/core.sh' --no-trap > /dev/null 2>&1; myvar=hello; othervar=world; dump_vars --force --quiet --name 'Custom Label' myvar othervar"
    assert_success
    assert_output --partial "Custom Label"
...
@test "dump_vars: --markdown and --graphical switch the table format" {
    run bash -c "source '$lib_dir/core.sh' --no-trap > /dev/null 2>&1; myvar=hello; dump_vars --force --quiet --markdown myvar"
    assert_success
    assert_output --partial "|"
    run bash -c "source '$lib_dir/core.sh' --no-trap > /dev/null 2>&1; myvar=hello; dump_vars --force --quiet --graphical myvar"
    assert_success
    assert_output --partial "║"
}

@test "dump_vars: --core-state dumps the internal core-state snapshot" {
    run bash -c "source '$lib_dir/core.sh' --no-trap > /dev/null 2>&1; dump_vars --force --quiet --core-state"
    assert_success
...
@test "dump_vars: --blank and --line render without crashing" {
    run bash -c "source '$lib_dir/core.sh' --no-trap > /dev/null 2>&1; myvar=hello; dump_vars --force --quiet myvar --blank --line myvar"
    assert_success
}

@test "dump_vars: restores the original quiet/verbose state after returning" {
    run bash -c "source '$lib_dir/core.sh' --no-trap > /dev/null 2>&1; myvar=hello; dump_vars --force --quiet myvar; is_verbose && echo verbose=true || echo verbose=false; is_quiet && echo quiet=true || echo quiet=false"
    assert_success
