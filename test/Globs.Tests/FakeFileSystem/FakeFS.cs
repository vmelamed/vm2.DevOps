...
public sealed partial class FakeFS
{
    ...
    public Folder SetCurrentFolder(string pathName)
    {
        ...
        if (folder is null)
            throw new ArgumentException($"Glob '{pathName}' not found in the JSON file system.", nameof(pathName));
        if (fileComp)
    ...
    Folder FromText(TextReader reader)
    {
        ...
        while (true)
        {
            ...
            if (!IsWindows)
            {
                ...
                line = UnixEnvVar().Replace(line, UnixEnvVarReplacement);
            }
